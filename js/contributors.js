...
const fetchData = async () => {
  await fetch('https://gdscjgec.github.io/Pictionary/data.json')
    .then((response) => {
  ...
  const htmlString = contributorsData
    .map((contributorData) => {
      return `
        ...
        <div class="data"><div class="list"><div class="number"><h1>${contributorData.contributions}</h1></div><div class="text"><p>Contributions</p></div></div></div><a href="${contributorData.html_url}" ><i class="fab fa-github"></i></a>
        </div>
        `;
    })
    .join('');
